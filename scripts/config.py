...
def parse():
  config_path = os.path.join(base.get_script_dir(), "..", "config")
  ...
  if ("windows" == host_platform) and ("" == option("vs-path")):
    ...
    if ("2015" == options["vs-version"]):
      options["vs-path"] = os.path.join(programFilesDir, "Microsoft Visual Studio 14.0", "VC")
    elif ("2019" == options["vs-version"]):
      vs_path_enterprise = os.path.join(programFilesDir, "Microsoft Visual Studio", "2019", "Enterprise", "VC", "Auxiliary", "Build")
      vs_path_professional = os.path.join(programFilesDir, "Microsoft Visual Studio", "2019", "Professional", "VC", "Auxiliary", "Build")
      vs_path_community = os.path.join(programFilesDir, "Microsoft Visual Studio", "2019", "Community", "VC", "Auxiliary", "Build")
      
      if base.is_dir(vs_path_enterprise):
        options["vs-path"] = vs_path_enterprise
      elif base.is_dir(vs_path_professional):
        options["vs-path"] = vs_path_professional
      else:
        options["vs-path"] = vs_path_community
  ...
  if check_option("use-system-qt", "1"):
    base.cmd_in_dir(os.path.join(base.get_script_dir(), "..", "tools", "linux"), "python", ["use_system_qt.py"])
    options["qt-dir"] = os.path.join(base.get_script_dir(), "..", "tools", "linux", "system_qt")
...
def check_compiler(platform):
  ...
  if base.host_platform() == "mac":
    if not base.is_dir(os.path.join(options["qt-dir"], compiler["compiler_64"])):
      if base.is_dir(os.path.join(options["qt-dir"], "macos")):
...
def parse_defaults():
  defaults_path = os.path.join(base.get_script_dir(), "..", "defaults")
  if ("" != option("branding")):
    defaults_path_branding = os.path.join(base.get_script_dir(), "..", "..", option("branding"), "build_tools", "defaults")
