...
def make():
  ...
  socket_cpp_dir = os.path.join(base_dir, "socket.io-client-cpp")
  
  if not base.is_dir(socket_cpp_dir):
    base.cmd_in_dir(base_dir, "git", ["clone", "https://github.com/socketio/socket.io-client-cpp.git"])
    base.cmd_in_dir(socket_cpp_dir, "git", ["checkout", "da779141a7379cc30c870d48295033bc16a23c66"])
    base.cmd_in_dir(socket_cpp_dir, "git", ["submodule", "init"])
    base.cmd_in_dir(socket_cpp_dir, "git", ["submodule", "update"])
    base.cmd_in_dir(os.path.join(socket_cpp_dir, "lib", "asio"), "git", ["checkout", "230c0d2ae035c5ce1292233fcab03cea0d341264"])
    base.cmd_in_dir(os.path.join(socket_cpp_dir, "lib", "websocketpp"), "git", ["checkout", "56123c87598f8b1dd471be83ca841ceae07f95ba"])
    # patches
    base.apply_patch(os.path.join(socket_cpp_dir, "lib", "websocketpp", "websocketpp", "impl", "connection_impl.hpp"), 
                     os.path.join(base_dir, "patches", "websocketpp.patch"))
    base.apply_patch(os.path.join(socket_cpp_dir, "src", "internal", "sio_client_impl.cpp"), 
                     os.path.join(base_dir, "patches", "sio_client_impl_fail.patch"))
    base.apply_patch(os.path.join(socket_cpp_dir, "src", "internal", "sio_client_impl.cpp"), 
                     os.path.join(base_dir, "patches", "sio_client_impl_open.patch"))
    base.apply_patch(os.path.join(socket_cpp_dir, "src", "internal", "sio_client_impl.cpp"), 
                     os.path.join(base_dir, "patches", "sio_client_impl_close_timeout.patch"))

    # no tls realization (remove if socket.io fix this)
    dst_dir = os.path.join(socket_cpp_dir, "src_no_tls")
    base.copy_dir(os.path.join(socket_cpp_dir, "src"), dst_dir)
    ...
    base.replaceInFile(os.path.join(socket_cpp_dir, "src", "internal", "sio_client_impl.cpp"), old_ping, new_ping)
    base.replaceInFile(os.path.join(socket_cpp_dir, "src_no_tls", "internal", "sio_client_impl.cpp"), old_ping, new_ping)
  return
